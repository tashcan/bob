...
template <typename T>
inline T get_config_or_default(toml::table config, toml::table& new_config, std::string_view section, std::string_view item, T default_value)
{
  ...
  message.str("");
  message << "        config value " << section << "." << item << "  type: " << get_config_type_as_string(config[section][item].type());
  spdlog::info(message.str());
...
void Config::Load()
{
  ...
  this->system_pan_momentum_falloff = get_config_or_default(config, parsed, "graphics", "system_pan_momentum_falloff", 0.8f);
  this->keyboard_zoom_speed         = get_config_or_default(config, parsed, "graphics", "keyboard_zoom_speed", 350.0f);
  this->borderless_fullscreen_f11   = get_config_or_default(config, parsed, "graphics", "borderless_fullscreen_f11", true);
  ...
  this->stay_in_bundle_after_summary = get_config_or_default(config, parsed, "ui", "stay_in_bundle_after_summary", true);
  ...
  std::string disabled_banner_types_str = get_config_or_default<std::string>(config, parsed, "ui", "disabled_banner_types", "");
